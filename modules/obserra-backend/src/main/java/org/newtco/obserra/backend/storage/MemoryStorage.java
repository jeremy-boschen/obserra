...
public class MemoryStorage implements Storage {
    ...
    private final Map<String, ObService>            services         = new ConcurrentHashMap<>();
    private final Map<String, List<Metric>>         metrics          = new ConcurrentHashMap<>();
    private final Map<String, List<ObLog>>          logs             = new ConcurrentHashMap<>();
    ...
    // Service methods
    @Override
    public List<ObService> getAllServices() {
        return new ArrayList<>(services.values());
    }

    @Override
    public Optional<ObService> getService(String id) {
        return Optional.ofNullable(services.get(id));
    }

    @Override
    public Optional<ObService> getServiceByPodName(String podName) {
    ...
    @Override
    public Optional<ObService> getServiceByAppId(String appId) {
        return services.values().stream()
    ...
    @Override
    public ObService createService(ObService service) {
        service.setLastUpdated(OffsetDateTime.now());
    ...
    @Override
    public ObService updateService(String id, ObService updatedService) {
        ObService existingService = services.get(id);
        if (existingService == null) {
            throw new IllegalArgumentException("Service not found with id: " + id);
        }

        existingService.update(updatedService)
            .setLastSeen(OffsetDateTime.MIN);

        return existingService;
    }

    @Override
    public ObService updateServiceByAppId(String appId, ObService service) {
    ...
    @Override
    public ObService updateServiceStatus(String id, ObServiceStatus status) {
        ObService service = services.get(id);
        if (service == null) {
            throw new IllegalArgumentException("Service not found with id: " + id);
        }
    ...
    @Override
    public ObService updateServiceLastSeen(String id) {
        ObService service = services.get(id);
        if (service == null) {
            throw new IllegalArgumentException("Service not found with id: " + id);
        }

        service.setLastSeen(OffsetDateTime.now());
        return service;
    }
    ...
    @Override
    public ObService persistServiceData(ObService service) {
    ...
    @Override
    public ObService registerService(ObService registration) {
        // Check if this service has already registered with an appId
        Optional<ObService> existingService = Optional.empty();
    ...
    @Override
    public List<ObService> getServicesForHealthCheck(int maxAgeSeconds) {
        OffsetDateTime cutoffTime = OffsetDateTime.now().minusSeconds(maxAgeSeconds);

        return services.values().stream()
            .filter(service -> {
                // Skip services that don't need checking
                if (service.getStatus() == ObServiceStatus.DOWN &&
                ...
                if (service.getCheckInterval() != null && service.getLastSeen() != null) {
                    OffsetDateTime nextCheckTime = service.getLastSeen()
                        .plus(service.getCheckInterval());
                    return OffsetDateTime.now().isAfter(nextCheckTime);
                }
    ...
    // Logs methods
    @Override
    public List<ObLog> getLogsForService(String serviceId, int limit) {
        List<ObLog> serviceLogs = logs.getOrDefault(serviceId, new ArrayList<>());

        // Return the most recent logs first
        return serviceLogs.stream()
            .sorted(Comparator.comparing(ObLog::getTimestamp).reversed())
            .limit(limit)
            .collect(Collectors.toList());
    }

    @Override
    public ObLog createLog(ObLog log) {
        log.setId(String.valueOf(currentLogId.getAndIncrement()));
        if (log.getTimestamp() == null) {
            log.setTimestamp(LocalDateTime.now());
        }

        List<ObLog> serviceLogs = logs.computeIfAbsent(log.getServiceId(), k -> new ArrayList<>());
        serviceLogs.add(log);
