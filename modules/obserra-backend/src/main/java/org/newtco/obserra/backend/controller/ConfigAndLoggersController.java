...
public class ConfigAndLoggersController {
    ...
    public ResponseEntity<?> getServiceLoggers(@PathVariable String id) {
        try {
            Optional<ObService> serviceOpt = storage.getService(id);
            ...
            Map<String, LegacyLogger> loggers = loggerService.getLoggers(service);
    ...
    public ResponseEntity<?> setServiceLoggerLevel(
        ...
        try {
            ...
            ObLoggerLevel loggerLevel;
            try {
                loggerLevel = ObLoggerLevel.valueOf(level);
