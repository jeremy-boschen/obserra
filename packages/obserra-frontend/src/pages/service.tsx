...
export default function ServicePage() {
  ...
  // Wrap the content in ServiceProvider with initialServiceId
  return (
    <ServicesProvider initialServiceId={id}>
      <ServiceDetailContent initialSections={initialSections} />
    </ServicesProvider>
  );
}

// This component uses the context provided by ServiceProvider
function ServiceDetailContent({ initialSections }: { initialSections: any }) {
  const [, setLocation] = useLocation();
  const { toast } = useToast();

  const {
    serviceDetails: service,
    isLoadingServiceDetails: isLoading,
    serviceDetailsError: error,
    refreshServiceDetails: refreshService,
  } = useServicesContext();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
          <p className="text-gray-600 dark:text-gray-300">
            The service could not be found. It may have been removed or is unavailable.
          </p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <ServiceDetail onBack={() => setLocation("/")} initialSections={initialSections} />
    </div>
  );
}
