...
export function FullscreenLogs({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col ${theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}
    >
      {/* Header */}
      <div
        className={`px-6 py-4 flex items-center justify-between border-b ${theme === "dark" ? "border-gray-700" : "border-gray-200"}`}
      >
      ...
      {/* Search and Filters */}
      <div
        className={`px-6 py-3 flex flex-col sm:flex-row gap-3 border-b ${theme === "dark" ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-gray-50"}`}
      >
        <div className="relative flex-grow">
          <Search
            className={`absolute left-2.5 top-2.5 h-4 w-4 ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}
          />
          <Input
            type="text"
      ...
      {/* Footer with stats */}
      <div
        className={`px-6 py-3 border-t ${theme === "dark" ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-gray-50"}`}
      >
            ...
            {["ERROR", "WARNING", "INFO", "DEBUG", "TRACE"].map((level) => {
              ...
              return (
                <div key={level} className="flex items-center text-sm">
                  <Badge variant="outline" className={cn("px-2 font-semibold mr-1", levelColor)}>
                    {level}
                  </Badge>
                  <span>{count}</span>
                </div>
