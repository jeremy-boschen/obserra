...
export function Sidebar({ sidebarOpen }: { sidebarOpen: boolean }) {
  ...
  const { activeServiceId, setActiveServiceId, services, autoRefresh, setAutoRefresh } =
    useServicesContext();

  const toggleActiveServiceId = (id: string) => {
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
          {services.map((service) => {
            ...
            return (
              <button
                key={service.id}
                onClick={() => toggleActiveServiceId(service.id)}
                className={cn(
              ...
              >
                <span
                  className={cn("inline-block w-2.5 h-2.5 rounded-full mr-2", statusColor.bg)}
                ></span>
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              ...
              <div className="mt-1">
                <span className="inline-flex items-center text-success-600 dark:text-success-500">
                  <span className="inline-block w-2.5 h-2.5 rounded-full mr-2 bg-success-500"></span>
                  {upServices} UP
                </span>
                <span className="ml-2 inline-flex items-center text-warning-600 dark:text-warning-500">
                  <span className="inline-block w-2.5 h-2.5 rounded-full mr-2 bg-warning-500"></span>
                  {warningServices} WARNING
                </span>
                <span className="ml-2 inline-flex items-center text-error-600 dark:text-error-500">
                  <span className="inline-block w-2.5 h-2.5 rounded-full mr-2 bg-error-500"></span>
                  {downServices} DOWN
                </span>
              </div>
            </div>
