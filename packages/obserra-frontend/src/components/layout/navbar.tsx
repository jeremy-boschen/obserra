...
import { TimeSince } from "@/components/ui/time-since.tsx";

interface NavbarProps {
  toggleSidebar: () => void;
}

export function Navbar({ toggleSidebar }: Omit<NavbarProps, "isRefreshing">) {
  const { loading, refresh, refreshedAt } = useServicesContext();
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Last refresh: <TimeSince value={refreshedAt} interval={5_000} />
            </div>
            <Button
              size="sm"
              onClick={refresh}
